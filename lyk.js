...
const digits = ["","", "kı","san","von","mum","hau","dıaı","roaı","neı"];

const numbermaker= (n) =>  {
  if (n===0){return "bur nó <b>dıt</b> són"}
  const s = n.toString();
  const maxpower = s.length-1;
  const result = [...s].map((d, i) => digits.at(d)+(d!=0 ? powers_of_ten[(maxpower - i) % 3]:"")).join(" ");


return `bur nó són <b>${result}</b>`; 
}
const renderEntry = (entry) => {
  ...
  return `
    <dl class="entry">
      ...
      <ul>
      ${entry.defs.map(x=>`<li><span>›&emsp;${x.def}</span>${x.sign?`<div class="sign">${x.sign}</div>`:""}</li>`).join("")}
      </ul></dd>
      
    </dl>`;
}

const desc = "kúan à ar já ban zhá - dictionary searcher for conlangs";

const rankEntries= (data, qRaw) =>  {
  const query = qRaw.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, "").replace("ı", "i");
  ...
  let a =  data
    .map(entry => {
      ...
      if (def.some(x=>x.split(" ").includes(qDef))) s += 5;
      else if (def.some(x=>x.includes(qDef))) s += 3;
      if (!s) return null;
...
export {DICT, desc, numbermaker,renderEntry, rankEntries};
